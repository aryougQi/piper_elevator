...
def test_visual_servo_has_safe_three_cm_contract():
    ...
    assert 0.0 < config['world_position_smoothing_alpha'] < 1.0
    assert 0.0 < config['world_normal_smoothing_alpha'] < 1.0
    assert config['button_selection_topic'] == '/button_selection'
    assert config['exact_tf_wait_seconds'] <= 0.03
    assert config['allow_latest_tf_fallback'] is False
    assert config['maximum_tf_fallback_age_seconds'] <= 0.02
...
def test_detector_publishes_only_fitted_surface_pose_for_servo():
    ...
    detector_config = yaml.safe_load(
        (PACKAGE_ROOT / 'config' / 'button_detector.yaml').read_text()
    ...
    assert detector_config['camera_position_smoothing_alpha'] == 1.0
    assert detector_config['surface_normal_smoothing_alpha'] == 1.0
    assert detector_config['sync_slop_seconds'] <= 0.02
    assert '_publish_surface_pose(' in detector
    # Recognition uses the restored strict-class legacy pipeline.
    assert 'filter_detections_by_class(' in detector
    assert '_publish_tracking_state(' in detector
    ...
    assert 'self._observation_anchor = None' in servo
    assert config['coarse_handover_service'] == '/button_approach_planner/claim_servo'
    assert '_claim_coarse_handover' in servo
    assert 'decode_coarse_handover' in servo
    assert "'~/claim_for_press'" in servo
    assert '_lookup_surface_transform' in servo
    assert 'self._filtered_world_position' in servo
    assert 'self._filtered_world_normal' in servo
...
def test_coarse_launch_policy_is_shared_before_visual_servo():
    real_launch = (
        PACKAGE_ROOT / 'launch' / 'button_approach_real.launch.py'
    ).read_text()

    assert "'constrain_coarse_orientation': 'true'" in real_launch
    assert (
        "'preserve_coarse_camera_orientation': 'false'" in real_launch
    )
    assert "'coarse_vertical_offset_m': LaunchConfiguration(" in real_launch
    assert "'preserve_wrist_roll_from_current': 'false'" in real_launch
    assert "'level_roll_enabled': 'true'" in real_launch
    assert "'perpendicular_tolerance_rad': '0.05236'" in real_launch
    assert "'axial_approach_stop_angle_rad': '0.05236'" in real_launch

    sim_launch = (
        PACKAGE_ROOT / 'launch' / 'button_approach_sim.launch.py'
    ).read_text()
    assert "'constrain_coarse_orientation': True" in sim_launch
    assert "'preserve_coarse_camera_orientation': False" in sim_launch
    assert "'coarse_vertical_offset_m': 0.0" in sim_launch
